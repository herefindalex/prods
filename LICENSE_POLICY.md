...
| Scope | Result |
| --- | --- |
| Linux/Windows Go release closure | 37 external modules: MIT, BSD-3-Clause, and Apache-2.0 only |
...
The MPL-2.0 module is `github.com/hashicorp/golang-lru/v2@v2.0.7`. It is reached through modernc build/test packages and is not linked into either `prods-linux-amd64` or `prods-windows-amd64.exe`. CI fails if it enters either release closure.
...
The gate checks audited hashes for `go.mod`, `go.sum`, `package.json`, and `pnpm-lock.yaml`; the Go release closure for Linux and Windows; npm license-group counts; the reviewed build-only exception; and notice coverage for every release module and production package. Any dependency input change requires a new review and a deliberate update to `licenses/audit.json` and `THIRD_PARTY_NOTICES.md`.
