...
function initialLocale(): SystemLocale {
	...
	if (requested?.toLowerCase() === "zh-tw") return "zh-TW";
	if (requested?.toLowerCase() === "en-us") return "en-US";
...
function SystemFrame({ mode, children }: { mode: SystemMode; children: React.ReactNode }) {
  ...
  return (
    <ConfigProvider locale={locale === "zh-TW" ? zhTW : enUS} theme={{ token: { colorPrimary: "#147985", borderRadius: 8, colorBgLayout: "#eef3f5" } }}>
      <main className={`system-shell system-${mode}`}>
        <header className="system-header">
          <Space className="system-header-content" align="center">
            ...
            <Select<SystemLocale> aria-label={commonText[locale].language} value={locale} onChange={changeLocale} options={[{ value: "en-US", label: "English" }, { value: "zh-TW", label: "繁體中文" }]} />
          </Space>
        </header>
        {children}
      </main>
...
function InstallerApp() {
  ...
  if (!state && loading) return <Spin tip={commonText[locale].loading} fullscreen />;
  if (!state) return <Alert type="error" showIcon message={commonText[locale].requestFailed} description={error} action={<Button onClick={() => void load()}>{commonText[locale].retry}</Button>} />;
  ...
  return (
    <Card title={copy.title}>
      <Space direction="vertical" className="system-stack">
        ...
        {error && <Alert closable type="error" showIcon message={commonText[locale].requestFailed} description={error} onClose={() => setError(undefined)} />}
...
function RecoveryApp() {
  ...
  const copy = recoveryText[locale];
  const load = async () => {
    setLoading(true);
    try {
...
function MaintenanceApp() {
  ...
  const [error, setError] = useState<string>();
  const copy = maintenanceText[locale];
  const load = async () => {
    try {
  ...
  return (
    <Card>
      {error ? <Alert type="error" showIcon message={commonText[locale].requestFailed} description={error} action={<Button onClick={() => void load()}>{commonText[locale].retry}</Button>} /> : !state ? <Spin /> : state.active ? (
